...
class Server
{
    ...
    std::vector<std::shared_ptr<User>> usersConnections;     // вектор хранения подключений пользователя (клиентов)
...
public:
    ...
    void socketError(std::shared_ptr<User> currentUser);                                             // обработка ошибки сокета
    void exitProcessing(std::shared_ptr<User> currentUser);                                          // обработка команды /exit
    void startProcessing(std::shared_ptr<User> currentUser);                                         // обработка команды /start
    void clientCountProcessing(std::shared_ptr<User> currentUser) const;                             // обработка команды /client_count
    void sendCommandProcessing(std::shared_ptr<User> currentUser, std::string sendCommandString);    // обработка команды /send_message
    void showCommandProcessing(std::shared_ptr<User> currentUser, std::string showCommandString);    // обработка команды /show_message 
};
...
void Server::serverReceive(SOCKET transmittedSocket)
{
    mtxNewConnect.lock();
    allUsersCounter++;
    std::shared_ptr<User> currentUser = std::make_shared<User>(transmittedSocket, allUsersCounter);
...
void Server::socketError(std::shared_ptr<User> currentUser)
{
...
void Server::exitProcessing(std::shared_ptr<User> currentUser)
{
...
void Server::startProcessing(std::shared_ptr<User> currentUser)
{
...
void Server::clientCountProcessing(std::shared_ptr<User> currentUser) const
{
...
void Server::sendCommandProcessing(std::shared_ptr<User> currentUser, std::string sendCommandString)
{
...
void Server::showCommandProcessing(std::shared_ptr<User> currentUser, std::string showCommandString)
{
